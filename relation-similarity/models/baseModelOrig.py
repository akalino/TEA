    ...
    def eval_step(self, rel, sample_ent):
        with torch.no_grad():
            ...
            for i in range(sample_ent):
                ...
                tmp2 = self.forward(
                    self.remb(torch.LongTensor([_ for _ in range(self.tot_rel)]).cuda()) + self.eemb(head),
                    self.layers2)
                pt = self.log_softmax(tmp2 - torch.max(tmp2, dim=-1, keepdim=True)[0])[
                    [_ for _ in range(self.tot_rel)], tail].cuda()
