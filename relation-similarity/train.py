...
if args['load']:
    model.load_state_dict(torch.load(os.path.join(args['output'], '{}-{}-model.pth'.format(ds_name, args['class']))))
    print("Model loaded")
    model.opt.load_state_dict(torch.load(os.path.join(args['output'],
                                                      '{}-{}-optim.pth'.format(ds_name, args['class']))))
    print("Optimizer loaded")
    with open(os.path.join(args['output'], '{}-{}-info.json'.format(ds_name, args['class']))) as data:
        info = json.load(data)
    epoch = info['epoch']
    print("Info loaded")
else:
...
while epoch < 500:
    ...
    if epoch % args['val_step'] == 0:
        ...
        if val_loss < min_val_loss:
            ...
            torch.save(model.state_dict(), os.path.join(args['output'],
                                                        '{}-{}-model.pth'.format(ds_name, args['class'])))
            torch.save(model.opt.state_dict(), os.path.join(args['output'],
                                                            '{}-{}-optim.pth'.format(ds_name, args['class'])))
    ...
    with open(os.path.join(args['output'], '{}-{}-info.json'.format(ds_name, args['class'])), 'w') as f:
        json.dump(info, f)
    epoch += 1
writer.export_scalars_to_json(os.path.join(args['output'], "{}-{}-all_scalars.json".format(ds_name, args['class'])))
writer.close()
