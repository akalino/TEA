...
class TripleEmbedder(nn.Sequential):
    ...
    :param patience: Number of consecutive epochs where validation score does not improve, default 3.
    ...
    def __init__(self, model_name_or_path: Optional[str] = None, modules: Optional[Iterable[nn.Module]] = None,
                 patience=3, device: Optional[str] = None, cache_folder: Optional[str] = None):
        self.patience = patience
        self._model_card_vars = {}
        self._model_card_text = None
        self._model_config = {}
        self.no_eval_improve = 0
        ...
        self.triple_embeddings = self.triple_mapper.triple_embeddings
    ...
    def encode(self, _inputs,
        ...
        if isinstance(_inputs, str) or not hasattr(_inputs,
        ...
        # all_embeddings = torch.empty((0, 256), dtype=torch.float32).to(device)
        all_embeddings = []
        for start_index in trange(0, len(_inputs), batch_size, desc="Batches", disable=not show_progress_bar):
            inputs_batch = _inputs[start_index:start_index+batch_size]
             # features = batch_to_device(inputs_batch, device)
            features = inputs_batch

            with torch.no_grad():
                out_features = self.eval_forward(features)
                ...
                for j in embeddings:
                    # all_embeddings = torch.cat((all_embeddings, torch.tensor(j)), 0)
                    all_embeddings.append(j.clone().detach().cpu())
    ...
    def eval_forward(self, trip1):
        a = self.triple_embeddings(torch.tensor(trip1).cuda())
        return a

    def encode2(self, sentences: Union[str, List[str]],
               batch_size: int = 32,
               show_progress_bar: bool = None,
               output_value: str = 'sentence_embedding',
    ...
    def save(self, path: str, model_name: Optional[str] = None, create_model_card: bool = True):
        ...
        for idx, name in enumerate(self._modules):
            ...
            try:
                ...
                embedding = module.get_weights()
                embedding.save(model_path)
                modules_config.append(
                    {'idx': idx, 'name': name, 'path': os.path.basename(model_path), 'type': type(embedding).__module__})
    ...
    def fit(self,
            ...
            scheduler: str = 'warmupcosinewithhardrestarts',
        ...
        for epoch in trange(epochs, desc="Epoch", disable=not show_progress_bar):
            ...
            iter_no_improve = self._eval_during_training(evaluator, output_path, save_best_model, epoch, -1, callback)
            if iter_no_improve >= self.patience:
                print('Early stopping occurred at epoch {}'.format(epoch))
    ...
    def _eval_during_training(self, evaluator, output_path, save_best_model, epoch, steps, callback):
        ...
        if evaluator is not None:
            ...
            if score > self.best_score:
                ...
                if save_best_model:
                    print('Best eval so far! Sim score: {}'.format(self.best_score))
                    # self.save(output_path)
            else:
                print('Performance degraded {}, may be overfitting'.format(self.best_score - score))
                self.no_eval_improve += 1
        return self.no_eval_improve
