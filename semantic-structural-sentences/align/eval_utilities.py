...
def find_similarities_annoy(_model, _valid_x, _valid_y, _n1, _n2):
    ...
    print('Annoy index {}'.format(len(preds_collect)))
    print('AC shape: {}'.format(actuals_collect.shape))
    f = len(actuals_collect[0])
    ...
    for pv_idx in tqdm(range(len(preds_collect))):
        ...
        ind_vecs_5.append(t.get_nns_by_vector(pred_vec.T, _n1, search_k=-1, include_distances=False))
        ind_vecs_10.append(t.get_nns_by_vector(pred_vec.T, _n2, search_k=-1, include_distances=False))
