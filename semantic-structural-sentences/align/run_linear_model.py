...
from align_models.PTSSMapper import PTSSMapper
...
def evaluate(_model, _valid_x, _valid_y, _shape, _valid_path, _w, _ann, _lm, _cn):
    print('Num valid batches {}'.format(len(_valid_x)))
    for j in range(len(_valid_x)):
        _valid_new = _model.forward(_valid_x[j].cuda().squeeze())
        print(_valid_new.shape)
        _valid_old = _model.prior_space(_valid_x[j].cuda().squeeze())
        print(_valid_old.shape)
        _valid_trg = _model.sent_embedding(_valid_y[j].cuda().squeeze())
        print(_valid_trg.shape)
        ...
        if _ann:
            p_5, p_10, valid_df = find_precision(_model, _valid_x[j].cuda(), _valid_y[j].cuda(), _valid_path, True)
        else:
            p_5, p_10, valid_df = find_precision(_model, _valid_x[j].cuda(), _valid_y[j].cuda(), _valid_path, False)
        if _w:
...
def run_triples(_sent_space, _triple_space, _agg, _norm,
                _train, _valid, _batch_sz,
                _init_lr, _epochs, _es_tol, _beta, _tune):
    _model = PTSSMapper(_sent_space, _triple_space,
    ...
    _valid_x, _valid_y = data.get_validation_batches()
    for e in tqdm(range(_epochs)):
        losses = []
        batch_x, batch_y = data.create_all_batches()
        for idx in tqdm(range(len(batch_x))):
            _model.orthogonalize()
            _model.zero_grad()
            if torch.cuda.is_available():
                inputs = Variable(batch_x[idx].cuda()).squeeze()
        ...
        for v_idx in range(len(valid_batch_x)):
            if torch.cuda.is_available():
                inputs = Variable(valid_batch_x[v_idx].cuda()).squeeze()
...
def experiment(_config_name):
    ...
    if 'ptss' in config['kg_embedding']:
        triple_emb_path = os.path.join(wd, 'kg-embeddings/triples/triples_{d}-{m}-ht-5_300_5.pt'.format(d=config['data_source'],
                                                                                                        m=config["model_type"]))
        triples = torch.load(triple_emb_path)
        print(triples.shape)
    else:
    ...
    if 'ptss' in config['kg_embedding']:
        model, valid_x, valid_y = run_triples(sent_emb, triples, 'ht',
    ...
    metrics, h5, h10, avg_sim = evaluate(model, valid_x, valid_y, config['compute_shape'], valid_path,
                                         False, True, config['language_model'], config_label)
    try:
